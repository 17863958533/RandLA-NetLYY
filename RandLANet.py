...
from tensorflow.python import debug as tf_debug   #Start Debug
...
class Network:
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                #-------------------------------------------
                # 这里是存储训练后的模型，我后来发现他只会存储mIOU最好的
                # 但这个不一定是最佳的，因此我把每一类检测物体当中的最小值大于0的也存储下来，以增加测试的可能性
                #-------------------------------------------
                ...
                #if m_iou > np.max(self.mIou_list) or np.min(self.mIou_list) > 0.00:
                    # Save the details model
                    snapshot_directory = join(self.saving_path, 'snapshots')
                    makedirs(snapshot_directory) if not exists(snapshot_directory) else None
                    self.saver.save(self.sess, snapshot_directory + '/snap', global_step=self.training_step)
    ...
    def evaluate(self, dataset):
        ...
        for step_id in range(self.config.val_steps):
            ...
            try:
                #self.prob_logits = tf.reshape(self.prob_logits, [-1, self.config.num_classes])  #xiugai !!!
                ...
                if not self.config.ignored_label_inds:
                    ...
                    labels_valid = labels_valid - 1   #去除无效标签，在Semantic3D数据集中标签为0的点是无效的，删除这些点后，其他点的标签值就要对应的减去1,因为模型预测结果中的0对应的是实际标签中的1。!!!
